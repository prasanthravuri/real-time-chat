...
import com.chatapp.real_time_chat.payload.JwtResponse;
import com.chatapp.real_time_chat.payload.LoginRequest;
import com.chatapp.real_time_chat.payload.MessageResponse;
import com.chatapp.real_time_chat.payload.SignupRequest;
import com.chatapp.real_time_chat.repository.RoleRepository;
import com.chatapp.real_time_chat.repository.UserRepository;
import com.chatapp.real_time_chat.security.jwt.JwtUtils;
import com.chatapp.real_time_chat.security.services.UserDetailsImpl;
...
public class AuthController {
    ...
    public ResponseEntity<?> authenticateUser(@Valid @RequestBody LoginRequest loginRequest) {
        ...
        try {
            ...
            if (!user.isEnabled()) {
                logger.error("❌ User disabled: {}", loginRequest.getUsername());
                return ResponseEntity.badRequest()
            ...
            logger.info("🎉 Login successful for user: {} with roles: {}", userDetails.getUsername(), roles);
            ...
            logger.error("💥 Unexpected error during login for user: {} - Error: {}", loginRequest.getUsername(), e.getMessage());
            return ResponseEntity.badRequest()
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) {
        ...
        try {
            ...
            if (strRoles == null) {
                ...
                roles.add(userRole);
                logger.info("✅ Assigned default ROLE_USER to: {}", signUpRequest.getUsername());
            } else {
            ...
            logger.error("💥 Registration failed for user: {} - Error: {}", signUpRequest.getUsername(), e.getMessage());
            return ResponseEntity.badRequest()
